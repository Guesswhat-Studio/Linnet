...
>
> 第一次部署完成后，Linnet 会尝试把你的 GitHub Pages 地址同步到仓库的 **About -> Website**。如果 GitHub 权限阻止了这一步，再手动把 Pages 地址填进去即可。
...
- 在权限允许时，把仓库 **About -> Website** 链接设为你的 Pages 地址
- 触发第一次 `Daily Digest`
...
Linnet 也会尝试把这个地址填到仓库主页的 **About -> Website** 里。如果设置完成后 About 里仍然指向模板仓库旧链接，再手动把它替换成你的 Pages 地址即可。
